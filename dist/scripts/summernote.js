...
(function (factory) {
    ...
    var dialog = renderer.create('<div class="modal" aria-hidden="false" tabindex="-1" data-type="summernote"/>', function ($node, options) {
        if (options.fade) {
            $node.addClass('fade');
        }
        $node.html([
            '<div class="modal-dialog">',
            '  <div class="modal-content">', (options.title ?
                '    <div class="modal-header">' +
                '      <h4 class="modal-title">' + options.title + '</h4>' +
                '      <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>' +
                '    </div>' : ''
            ),
            '    <div class="modal-body">' + options.body + '</div>', (options.footer ?
    ...
    var checkbox = renderer.create('<label class="custom-control custom-checkbox"></label>', function ($node, options) {
        ...
        $node.html([
            ' <input type="checkbox" class="custom-control-input"' + (options.id ? ' id="' + options.id + '"' : ''), (options.checked ? ' checked' : '') + '/>',
    ...
    var Handle = function (context) {
        ...
        this.initialize = function () {
            this.$handle = $([
                ...
                '<div class="', (options.disableResizeImage ? 'note-control-holder' : 'note-control-sizing'),
                ' note-control-se"></div>', (options.disableResizeImage ? '' : '<div class="note-control-selection-info"></div>'),
                '</div>',
                '</div>'
...
//                ' type="file" name="files" accept="image/*" multiple="multiple" />' +
                imageLimitation +
                '</div>';
            if (typeof(updateImageSelector) == 'function') {
                body += '<select id="summernoteImageFolder" class="form-control form-control-sm" onchange="updateImageSelector($(this), $(this).parent().find(\'.ImageFiles\'))"/>'
                    + '<div id="summernoteImageFiles" class="ImageFiles"></div>';
            }
    ...
    $.summernote = $.extend($.summernote, {
        ...
        options: {
            ...
            fontSizes: ['7', '8', '9', '10', '11', '12', '14', '18', '24', '36'],
