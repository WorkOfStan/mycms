...
class App extends MyCommon
{
    ...
    public function run()
    {
        ...
        if (is_array($this->post) && !empty($this->post)) {
            ...
            $this->MyCMS->getSessionLanguage($this->get, $this->session, true); // todo check if it is really necessary here
        ...
        $this->MyCMS->WEBSITE = $this->WEBSITE[$this->session['language']]; // language is already properly set through FriendlyURL mechanism
        Debugger::barDump(
...
//        Assert::nullOrIsArray($this->session['token']);
